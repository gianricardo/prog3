/*
 * estadojogo.cpp
 *
 *  Created on: 19 de jun de 2017
 *      Author: lucas
 */

#include "estadojogo.h"

namespace p3 {

Estado_jogo::Estado_jogo() {

}

Estado_jogo::~Estado_jogo() {

}

static bool Estado_jogo::inicia_jogo(std::string nome) : Jogo(new R_Paciencia(), std::vector<std::string>(1, nome)){
	for(int i = 0; i < 12; i++){
		Jogo::novo_monte();
	}

	return Estado_jogo::distribuir();

}

bool Estado_jogo::realiza_jogada(int p_m1, int p_m2){

	if(p_m1 == 0){

		Jogo::vira_carta_monte(0, true);
		Jogo::move_carta_mm(0, 1, true, true);

		return true;
	}
	else if (p_m1 >= 6 && p_m1 <= 12 && p_m2 >= 2 ){

		Carta c1, c2;

		c1 = Estado_jogo::_pega_monte(p_m1);
		c2 = Estado_jogo::_pega_monte(p_m2);

		if(p_m2 <=5){

			if( (int) _mesa.tamanho_monte(p_m2) > 0){

				if(!(R_Paciencia::verifica_ordem(c1.numero(), c2.numero()) && R_Paciencia::verifica_naipe(c1.naipe(), c2.naipe())))
					return false;
			}

			Jogo::move_carta_mm(p_m1, p_m2);
			Jogo::jogador_soma_pontos(1, 0);

			Jogo::verifica_fim_de_jogo();

			return true;

		}
		else if(R_Paciencia::verifica_naipe(c1.naipe(), c2.naipe())){

			Jogo::move_carta_mm(p_m1, p_m2);

			return true;
		}
	}

	return false;
}


bool Estado_jogo::fim_jogo(){

	if(Jogo::_jogando == false)
		return true;

	return false;
}



bool Estado_jogo::distribuir(){

	int k = 1;
	bool a = true;

	for(std::size_t i = 6; i <= 12; i++){

		for(int j = 0; j < k;j++){

			if(Jogo::move_carta_mm((std::size_t) 0, i) == false){

				a = false;
			}
		}

		Jogo::vira_carta_monte(i, true);

		k++;
	}

	return a;
}

/*
int Estado_jogo::pontuacao(){

	return Jogo::pontuacao_jogador_atual();
}
*/

void Estado_jogo::restaura_monte_inicial(){

	unsigned long int tam = _mesa.tamanho_monte((unsigned long int)1), i;

	bool a;

	for(i = 0; i < tam; i++)
		a = Jogo::move_carta_mm((std::size_t) 1, (std::size_t) 0);

}

Carta* Estado_jogo::_pega_monte(std::size_t m){

	return Jogo::_mesa.verifica_topo(m); //verificar
}


} /* namespace p3 */
